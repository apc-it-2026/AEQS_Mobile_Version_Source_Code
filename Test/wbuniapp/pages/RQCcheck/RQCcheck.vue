<template>
    <div class='RQCcheck'>
        <u-navbar border :title="query.fac? $t('common.RQC审核工厂端') : $t('common.RQC审核') " :leftText="$t('common.返回')" placeholder autoBack>
            <u-button v-if="!query.fac" type="success" size="small" icon="plus" slot="right" :text="$t('common.新增')" @click="add()"></u-button>
        </u-navbar>
        <u-collapse :value="$t('common.搜索筛选')" accordion>
            <u-collapse-item :title="$t('common.搜索筛选')" :name="$t('common.搜索筛选')">
                <div class="myList">
                    <div class="item2">
                        <div class="label">{{$t('common.搜索关键字')}}</div>
                        <div class="value">
                            <u-input v-model="searchForm.keyword" clearable> </u-input>
                        </div>
                    </div>
                    <div class="item2">
                        <div class="label">{{$t('common.审核状态')}}</div>
                        <div class="value">
                            <mySelect :list="stateList" v-model="searchForm.stateObj" type="action"></mySelect>
                        </div>
                    </div>
                    <div class="item2">
                        <div class="label">{{$t('common.日期')}}</div>
                        <div class="value">
                            <uni-datetime-picker :start-placeholder="$t('common.开始日期')"  :end-placeholder="$t('common.结束日期')" v-model="searchForm.date" type="daterange" :rangeSeparator="$t('common.至')" @change="val=>searchForm.date=val" />
                        </div>
                    </div>
                    <div class="item2">
                        <div class="label"></div>
                        <div class="value">
                            <u-button type="primary" color="#0ab99c" icon="scan" :text="$t('common.扫码')" @click="scan()"></u-button>
                        </div>
                    </div>
                    <div class="item2">
                        <div class="label"></div>
                        <div class="value btnList">
                            <u-button class="btn" size='small' color="#909399" type="warning" @click="reset" :text="$t('common.重置筛选')"></u-button>
                            <u-button class="btn" size='small' type="primary" @click="onSearch" :text="$t('common.搜索')"></u-button>
                        </div>
                    </div>
                </div>
            </u-collapse-item>
        </u-collapse>
        <!--列表  -->
        <div class="content p15">
            <list ref="list" :option="option" :otherData="otherData">
                <template v-slot="{item}">
                    <div class="myList2 box" @click="toPage('edit',item)">
                        <div class="item" v-show="!query.fac">
                            <div class="label">{{$t('common.任务编号')}}:</div>
                            <div class="value">{{item.task_no}}</div>
                        </div>
                        <div class="item">
                            <div class="label">{{$t('common.日期')}}:</div>
                            <div class="value">{{item.createdate}}</div>
                        </div>
                        <div class="item">
                            <div class="label">{{$t('common.供应商')}}:</div>
                            <div class="value">{{item.suppliers_name}}</div>
                        </div>
                        <div class="item">
                            <div class="label">{{$t('common.审核类型')}}:</div>
                            <div class="value">{{item.audit_type_name}}</div>
                        </div>
                        <div class="item" v-show="!query.fac">
                            <div class="label">{{$t('common.项目总分')}}:</div>
                            <div class="value">{{item.item_total}}</div>
                        </div>
                        <div class="item">
                            <div class="label">{{$t('common.审核评分')}}:</div>
                            <div class="value">{{item.audit_score}}</div>
                        </div>
                        <div class="item">
                            <div class="label">{{$t('common.待改善问题点数')}}:</div>
                            <div class="value">{{item.stay_perfect_count}}</div>
                        </div>
                        <div class="item">
                            <div class="label">{{$t('common.审核状态')}}:</div>
                            <div class="value">{{item.audit_state}}</div>
                        </div>
                        <div class="item">
                            <div class="label">{{$t('common.审核人员')}}:</div>
                            <div class="value">{{item.createby}}</div>
                        </div>
                    </div>
                </template>
            </list>
        </div>
    </div>
</template>
<script>
export default {
    name: 'RQCcheck',
    data() {
        return {
            query: {},
            searchForm: {//搜索数据
                keyword: '',
                stateObj: {
                    label: '',
                    value: '',
                },
                date: []
            },
            stateList: [//审核状态
                {
                    label: this.$t('common.待改善'),
                    value: '0',
                },
                {
                    label: this.$t('common.已结案'),
                    value: '1',
                },
            ],
            option: {
                getDataName: 'RQCcheck_GetRQCAudit_Main',//获取列表数据的方法名
                delName: 'RQCcheck_DeleteRQCAudit',//删除数据的方法名
                delData: {//删除数据需要的参数
                    prop: 'task_no',//删除数据的对应字段名
                    value: 'task_no',//删除数据的对应值的字段名
                },
                page: {//page对应的参数
                    page: 'pageIndex',//页码对应字段名
                    pageRow: 'pageSize',//页尺寸的字段名
                },
            },
        }
    },
    computed: {
        otherData: function () {
            return {
                "keyword": this.searchForm.keyword,
                "start_date": this.searchForm.date[0] || '',
                "end_date": this.searchForm.date[1] || '',
                "state": this.searchForm.stateObj.value || '',
            }
        }
    },
    methods: {
        reset() {//重置筛选
            this.searchForm = uni.$u.deepClone(this.oldSearchForm)
            this.onSearch()
        },
        onSearch() {//搜索
            this.$nextTick(() => this.$refs.list.init())
        },
        add() {//新增
            this.toPage('add', '')
        },
        toPage(type, item) {
            uni.navigateTo({ url: `/pages/RQCcheck/RQCcheckDetail?type=${type}&task_no=${item.task_no}&fac=${this.query.fac}` })
        },
        scan() {//扫码
            console.log('扫描');
            // 允许从相机和相册扫码
            let _this = this
            uni.scanCode({
                success: function (res) {
                    console.log('条码类型：' + res.scanType);
                    console.log('条码内容：' + res.result);
                    _this.searchForm.keyword = res.result
                    _this.onSearch()
                }
            });
        },

    },
    onLoad(query) {
        this.oldSearchForm = uni.$u.deepClone(this.searchForm)
        this.query = query
        this.option.getDataName = this.query.fac ? 'RQCcheck_GetRQCAudit_Main_factory' : 'RQCcheck_GetRQCAudit_Main'
    },
    onShow() {
        this.onSearch()
    }
}
</script>
<style lang='scss' scoped>
.RQCcheck {
}
</style>
