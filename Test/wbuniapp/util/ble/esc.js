...
var app = getApp();
let BLEInformation = uni.getStorageSync('bluetoothInfo')
var jpPrinter = {
  createNew: function () {
    var jpPrinter = {};
    var data = [];
    ...
    jpPrinter.setLang = function (n) { //设置语言字符集
      // 0 美国(U.S.A.)
      // 1 法国(France)
      // 2 德国(Germany)
      // 3 英国(U.K.)
      ...
      // 13 韩国(Korea)
      // 14 斯洛文尼亚 / 克罗地亚(Slovenia / Croatia)
      // 15 中国(China)
      data.push(27)
      data.push(82)
      data.push(n)
    ...
    jpPrinter.setAbsolutePrintPosition = function (nl, nh) { //设置绝对打印位置
      data.push(27)
      data.push(36)
    ...
    jpPrinter.setToPageTpye = function (row) { //设为页模式
      data.push(27)
      data.push(76)
    }
    jpPrinter.printInPage = function (row) { //页模式下打印
      data.push(27)
      data.push(12)
    }
    jpPrinter.setDirectionInPage = function (row) { //在页模式下选择打印区域方向
      data.push(27)
      data.push(84)
      data.push(row)
    }
    jpPrinter.setLReInPage = function (nL, nH) { //页模式下设置纵向相对位置
      data.push(29)
      data.push(92)
      data.push(nL)
      data.push(nH)
    }
    jpPrinter.setLAbInPage = function (nL, nH) { //页模式下设置纵向绝对位置
      data.push(29)
      data.push(36)
      data.push(nL)
      data.push(nH)
    }
    jpPrinter.printToLine = function () { //打印到分割线
      data.push(29)
      data.push(12)
    }
    jpPrinter.setPlaceInPage = function (xL, xH, yL, yH, dxL, dxH, dyL, dyH) { //页模式下设置打印区域
      data.push(27)
      data.push(87)
      data.push(xL)
      data.push(xH)
