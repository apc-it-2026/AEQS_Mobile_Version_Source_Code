    ...
    <view class="u-upload" :style="[$u.addStyle(customStyle)]">
        <view class="u-upload__wrap" v-if="previewImage">
            <view class="u-upload__wrap__preview" v-for="(item, index) in lists" :key="index">
                <image v-if="item.isImage || (item.type && item.type === 'image')" :src="item.thumb || item.url" :mode="imageMode" class="u-upload__wrap__preview__image" @tap="onPreviewImage(item)" :style="[{
            ...
            <template v-if="isInCount">
                ...
                <view v-else class="u-upload__button" :hover-class="!disabled ? 'u-upload__button--hover' : ''" hover-stay-time="150" @tap="chooseFile" :class="[disabled && 'u-upload__button--disabled']" :style="[{
						width: $u.addUnit(width),
						height: $u.addUnit(height)
					}]">
    ...
    methods: {
        ...
        onPreviewImage(item) {
            ...
            uni.previewImage({
                ...
                fail() {
                    uni.$u.toast(uni.$vue.$t('common.预览图片失败'))
                },
            });
        },
